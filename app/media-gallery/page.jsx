...
import { 
  LuPlay, LuMonitor, LuMic, LuUsers, LuChevronRight, LuFileDown, LuX 
} from "react-icons/lu";
...
const videoLibrary = [
  {
    ...
    description: "Deep dive into SEBI regulations, history, and the future of Real Estate Investment Trusts.",
    category: "Masterclass",
    icon: <LuMonitor size={14} />,
    thumbnail: "https://img.youtube.com/vi/gc9ZAfi2C_w/maxresdefault.jpg",
  ...
  },
  {
    ...
    description: "Strategies for scaling business operations across 100+ cities in India.",
    category: "Masterclass",
    icon: <LuMonitor size={14} />,
...
const galleryItems = [
  ...
  { id: 5, type: 'image', size: 'small', title: 'Leadership Summit', category: 'Impact', src: '/Harshjasrotia.com/Ritesh/Photos/P_K00191.JPG' },
   { id: 6, type: 'image', size: 'large', title: 'Pan-India Operations Scale', category: 'Growth', src: '/Harshjasrotia.com/Ritesh/Photos/P_K00144.JPG' },
  { id: 7, type: 'image', size: 'small', title: 'Boardroom Strategy', category: 'Leadership', src: '/Harshjasrotia.com/Ritesh/Photos/DSC02314.JPG' },
  { id: 8, type: 'image', size: 'small', title: 'EdTech Training Workshop', category: 'Education', src: '/Harshjasrotia.com/Ritesh/Photos/DSC02352.JPG' },
  { id: 9, type: 'image', size: 'medium', title: 'Real Estate Sales Module', category: 'Real Estate', src: '/Harshjasrotia.com/Ritesh/Photos/DSC02368.JPG' },
  { id: 10, type: 'image', size: 'small', title: 'Leadership Summit', category: 'Impact', src: '/Harshjasrotia.com/Ritesh/Photos/P_K00191.JPG' },
];
    ...
    <main className="bg-white min-h-screen font-sans selection:bg-[#cc0000] selection:text-white">
      ...
      <AnimatePresence>
        {activeVideo && (
          ...
          >
            <button onClick={() => setActiveVideo(null)} className="absolute top-6 right-6 text-white hover:text-[#cc0000] transition-colors z-[1010]">
              <LuX size={40} />
            </button>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full max-w-6xl aspect-video rounded-3xl overflow-hidden shadow-2xl bg-black">
      ...
      <section className="relative h-[60vh] md:h-[80vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0 opacity-90">
          <img src="/Harshjasrotia.com/Ritesh/Photos/DSC02216.JPG" className="w-full h-full object-cover" alt="Media Background" />
        </div>
        <div className="absolute inset-0 bg-black/40" />
        <div className="relative z-20 text-center px-6">
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="inline-block px-5 py-2 bg-[#cc0000] text-white text-[10px] font-black tracking-[0.4em] rounded-full mb-8 shadow-2xl ">Press & Media Kit</motion.div>
          <h1 className="text-5xl md:text-8xl font-black text-white tracking-tight leading-none mb-10 ">Strategic <span className="text-[#cc0000]">Footprints.</span></h1>
          <div className="flex justify-center">
            <Link href="#" className="relative overflow-hidden rounded-md border-2 bg-[#cc0000] border-[#cc0000] px-10 py-4 font-black text-xs tracking-[0.2em] text-white group transition-all">
              <span className="absolute inset-0 w-0 bg-white transition-all duration-300 group-hover:w-full"></span>
              <span className="relative z-10 group-hover:text-[#cc0000] flex items-center gap-2   font-bold"><LuFileDown size={16} /> Download Media Bio (PDF)</span>
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {videoLibrary.slice(0, visibleCount).map((video) => (
              <motion.div key={video.id} layout whileHover={{ y: -12 }} className="group relative bg-white rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-xl transition-all duration-500">
                <div className="relative h-72 overflow-hidden">
                  ...
                  <div className="absolute top-6 left-6 flex items-center gap-2 bg-white/10 backdrop-blur-xl px-4 py-2 rounded-xl text-white text-[10px] font-black tracking-widest border border-white/20 ">{video.icon} {video.category}</div>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <button onClick={() => setActiveVideo(video.videoUrl)} className="w-15 h-15 bg-[#cc0000] rounded-2xl flex items-center justify-center text-white shadow-2xl hover:scale-110 transition-transform">
                      <LuPlay fill="white" size={28} className="ml-1" />
                    </button>
                  </div>
                </div>
                <div className="p-5">
                  <h3 className="text-2xl font-black text-[#222222] mb-4  tracking-tight group-hover:text-[#cc0000] transition-colors">{video.title}</h3>
                  <p className="text-[#222222] text-m leading-relaxed ">{video.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
